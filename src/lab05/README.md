...
## Технические особенности реализации стратегий

Для обеспечения гибкости и масштабируемости системы в проекте использованы два различных подхода к реализации стратегий:

# 1. Фабрика функций и Замыкания (`limit_filter`)
Функция **`limit_filter(min_amount)`** является "фабрикой", которая создает предикаты для фильтрации данных:
*   **Механизм**: Используется концепция **замыкания (closure)** — внутренняя функция "запоминает" значение `min_amount` из внешней области видимости даже после того, как внешняя функция завершила работу.
*   **Применение**: Позволяет динамически генерировать фильтры с любым порогом суммы без дублирования кода.

# 2. Callable-объекты как стратегии (`InterestStrategy`)
Класс **`InterestStrategy`** реализует паттерн «Стратегия» через механизм вызываемых объектов:
*   **Инициализация**: Через метод `__init__` объект гибко настраивается (например, устанавливается конкретный процент бонуса).
*   **Магический метод `__call__`**: Позволяет экземпляру класса вести себя как обычная функция. Это дает возможность передавать объект в методы высшего порядка (например, в `apply()`).
*   **Преимущество**: В отличие от простой функции, такой объект может хранить сложное состояние и настройки, сохраняя при этом удобный интерфейс вызова.
---
